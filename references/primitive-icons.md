...
面向 IT / 云 / 架构图的单色 24×24 图标库。每个图标都用 `currentColor`，从所属 SVG 继承 ink 色，自动适配编辑皮肤或任何 onboarding 进来的品牌色板。

## 用法

按名字找图标（下面每个 `### 名称` 标题），把代码块里的 `<svg>` 片段复制进图里。默认 24×24；用 `<g transform="translate(x,y) scale(s)">` 包裹以定位和缩放。颜色通过父组 / SVG 上的 `color`、`fill` 或 `stroke` 控制。

通用图标是描边风格（1.5px 细线，与技能其余部分一致）；品牌剪影是填充风格。同一张图里不要无故混用两种风格。
...
Source: Tabler Icons / `laptop` (MIT)
...
Source: Tabler Icons / `desktop` (MIT)
...
Source: Tabler Icons / `container` (MIT)
...
Source: Tabler Icons / `internet` (MIT)
...
Source: Tabler Icons / `firewall` (MIT)
...
Source: Tabler Icons / `load-balancer` (MIT)
...
Source: Tabler Icons / `gateway` (MIT)
...
Source: Tabler Icons / `backup` (MIT)
...
Source: Tabler Icons / `service` (MIT)
...
Source: Tabler Icons / `deployment` (MIT)
...
Source: Tabler Icons / `ingress` (MIT)
...
Source: Tabler Icons / `volume` (MIT)
...
Source: Tabler Icons / `request` (MIT)
...
Source: Tabler Icons / `response` (MIT)
...
Source: Tabler Icons / `pipeline` (MIT)
...
Source: Tabler Icons / `monitoring` (MIT)
...
Source: Tabler Icons / `docker` (MIT)
...
Source: Tabler Icons / `terraform` (MIT)
...
Source: Tabler Icons / `github` (MIT)
...
Source: Simple Icons / `postgres` (CC0)
...
Source: Tabler Icons / `active-directory` (MIT)
...
Source: Simple Icons / `sqlserver` (CC0)
...
Source: Simple Icons / `nifi` (CC0)

### airflow
Apache Airflow 调度器 / DAG 运行器。
...
Source: Direct fetch `pentaho`（cdn.worldvectorlogo.com）——使用前核实许可
...
Source: Direct fetch `dagster`（cdn.prod.website-files.com）——使用前核实许可
...
Source: Simple Icons / `superset` (CC0)
...
Source: Direct fetch `sas`（upload.wikimedia.org）——使用前核实许可
...
Source: Direct fetch `stata`（icon.icepanel.io）——使用前核实许可

### rstudio
RStudio / Posit（R 与 Python 的 IDE）。
...
Source: Tabler Icons / `txt` (MIT)

---

## 命名对照（旧名 → 新名）

本目录早期直接沿用来源库的原始命名，现统一为资产侧短名（assets/icons.html 与各 type 文档一致）。旧名按此表对回新名；来源库内部的原名以「许可致谢」所列各库为准。

- 设备与通用：device-laptop → laptop · device-mobile → phone · device-desktop → desktop · package → container · cube → vm · bolt → cache · stack-2 → queue · file-text → log · device-floppy → backup · device-sd-card → volume
- 网络与安全：world → internet · world-www → cdn · wall → firewall · shield-lock → vpn · arrows-split → load-balancer · door-enter → gateway · tag → dns · address-book → active-directory · user-shield → admin
- Kubernetes：hexagon → pod · topology-star → node · world-cog → service · rocket → deployment · arrow-right-rhombus → ingress
- 动作与 DevOps：braces → api · arrow-right → request · arrow-left → response · refresh → sync · alert-triangle → alert · git-merge → pipeline · chart-line → monitoring · test-pipe → test
- 品牌与数据栈：brand-docker → docker · brand-terraform → terraform · brand-aws → aws · brand-azure → azure · brand-github → github · googlecloud → gcp · postgresql → postgres · microsoftsqlserver → sqlserver · apachehive → hive · apachenifi → nifi · apacheairflow → airflow · apachesuperset → superset
- 统计与文件格式：spss-plain → spss · rstudio-plain → rstudio · file-type-xls → excel · file-type-csv → csv · file-type-txt → txt
...
各库许可均允许再分发（含于本仓库的 MIT 源码中）。品牌商标归各自所有；本图标集仅用于文档与说明性展示。