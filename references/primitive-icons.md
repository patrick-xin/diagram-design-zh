...
面向 IT / 云 / 架构图的单色 24×24 图标库。每个图标都用 `currentColor`，从所属 SVG 继承 ink 色，自动适配皮肤层或任何 onboarding 进来的品牌色板。
...
Source: Direct fetch `hop`（hop.apache.org）——Apache License 2.0（ASF 图形 logo 授权政策）
...
Source: Direct fetch `sas`（upload.wikimedia.org）——PD-textlogo 公有领域（Wikimedia Commons）
