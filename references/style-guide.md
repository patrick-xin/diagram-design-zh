...
- 思源两族已自托管（`fonts/` 内 GB2312 级 woff2 子集），**不外链 Google Fonts**——国内拉不到且破坏单文件契约。字体由系统三栈渲染（**兜底项不许删**）；**每张产出交付前固定跑 embed_fonts.py 内嵌用字子集**（图是拿来分发的——无字库、断网机器也要像素级正确；SKILL.md §7.1）。
...
- 存量示例（`assets/example-*.html`）制于字号校准之前，其中 10–11px 中文小字是旧坡道；新产出按本表，不照抄示例字号。
...
| 角色 | 用途 | 默认值（浅色） | 深色档 |
|---|---|---|---|
| `paper` | 页面背景、节点底 | `#ffffff` | `#29314f` |
| `paper-2` | 容器底（少用）、深色下 backend 节点填充 | `#f3f2ff` | `#13182e` |
| `ink` | 主文本、主描边 | `#29314f` | `#faf8ff` |
| `muted` | 次文本、默认箭头 | `#565e7e` | `#a2a9ce` |
| `soft` | 子标签、边界标签 | `#8f94ab` | `#787fa2` |
| `rule` | 发丝线 | `rgba(41,49,79,0.10)` | `rgba(250,248,255,0.12)` |
| `accent` | 焦点，≤2 处 | `#1a4dd9` | `#7d98ff` |
| `accent-tint` | 焦点底色 | `rgba(26,77,217,0.08)` | `rgba(125,152,255,0.10)` |
| `link` | HTTP/API 调用 | `#1a4dd9` | `#7d98ff` |

### 深色反转规则（浅 → 深）

浅色里一切 **ink 基**的 `rgba(41,49,79, X)` 在深色里翻转为**纸色基** `rgba(250,248,255, X)`——透明度不变，RGB 互换。accent 在深色纸面上略提亮（`#1a4dd9` → `#7d98ff`）保对比度；`backend` 节点的 `#ffffff` 填充换成 `paper-2`（`#13182e`），纯白在深色底上会刺眼。箭头标签遮罩、掩膜 rect 的填充色跟随 `paper` 换档（`#29314f`）。

**深色是 opt-in 变体**：用户点名深色 / 暗色 / 夜间模式，或投放目标本身是深色站点 / 深色幻灯时启用，模板用 [`assets/template-dark.html`](../assets/template-dark.html)。默认仍是浅色。对比度约束（ink 对 paper 达 WCAG AA）两档同守——深色档的取值已实算过。
...
跨类型**恒定语义**的五个实色。与系列色板的分工：系列色编码「分类」——映射任意、图例为准、换肤可重派；语义色编码「含义」——铁锈红在任何图里都读作安全。用于 dp-integration / it-state / data-flow 告警 / medallion staging 这类带领域角色的元素；普通装饰填充轮不到它们（那是 token / α 梯子的事）。

| Token | 值 | 语义 | 典型位置 |
|---|---|---|---|
| `sem-security` | `#b85450`（铁锈红） | 安全 / 身份 / 权限 / 审计 | dp 矩阵安全行、data-flow 告警芯片、统一身份底栏 |
| `sem-observability` | `#5a7d9a`（岩蓝） | 可观测 / 质量 / 监控 | 集中日志、校验层、监控闸 |
| `sem-governance` | `#7a8c47`（橄榄绿） | 治理 / 血缘 / 发布 | 治理条、血缘图、数据产品层 |
| `sem-backup` | `#8c6d3f`（暖棕） | 备份 / 归档 / 冷存 | 备份通道、冷层 |
| `sem-workspace` | `#c9a23a`（金） | 分析 / 工作区 / 沙盒 | medallion staging 层 |

α 派生与 token 同规则（现行梯子：填充 @0.06、描边 @0.45；离群档随 α 档位归并收编）。**语义色不吃换肤**——皮肤表换气质（token / 系列 / α），语义五色跨皮肤恒定；这是「语义化」的底线：换皮不换义。
...
| `@0.03` | external 填充 / 嵌套第二层底 |
| `@0.05` | store 填充 / security(accent) 填充 / 外部源卡 |
| `@0.06` | 语义族填充（sem-* 专用） |
...
| `@0.15` | 柱体 / 甘特条体（muted） |
| `@0.18` | 系列面积 / 层头带 / 告警芯片 |
| `@0.20` | optional 描边（虚线）/ 分隔线 / 散点点阵（muted） |
...
**枚举闭合的图表特例**：treemap 深度坡道 = ink@`{0.04, 0.07, 0.09, 0.11, 0.13, 0.16}` 六档整组使用（值编码面积深度，不许单抽一档）；深色三级文字 = 纸色基 `@0.72`（tonex 路线契约）。深色档各基色沿用同一套档位，RGB 按深色反转规则换基。
...
**可选皮肤「新中式」**（宣纸 / 玄墨 / 朱砂 / 黛蓝 + 钤印 / 题签 / 朱批）：见 [skin-xinzhongshi.md](skin-xinzhongshi.md)。token 整体替换，排版纪律不变。新中式暂只有浅色档——深色反转规则只适用于默认皮肤，中式深色档留待后续版本。
