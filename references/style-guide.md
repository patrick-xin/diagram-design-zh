...
字重梯子即上表四档（400 正文 / 500 标签角标 / 600 节点名标题）+ 一个类型级档：**700 仅数据类型芯片码字**（LS/DB/TB/FL/WB，mono 7px 反白压色块，见 type-data-flow §8）与终端皮肤页面标题（mono 加粗）。别处不用 700。
...
- **整条锚位**：图例分隔线距图内最下内容元素的底 **56 单位**——量内容底，不量轴线 / 网格（见「容器对齐与画布基线」）。默认取精确 56；±4 容差只在与其他几何协调时使用。
- **图例条内部坐标豁免 4px 网格**：基线 −18 / 块 −10 / 线样 −4 的偏移组合数学上不可能同时为 4 的倍数——分隔线与色块 y 落 4 的倍数，基线与线样允许 ≡2 (mod 4)。
...
- **内容四贴**：x 左贴 0、y 顶贴 0（顶部留白归 HTML margin 管）。绘图区按类型对齐家族惯例：bar 系 80..960（左轴列填满 0..80——竖排轴题 + 刻度）；散点左三列式（竖排轴题贴 x=0 → 刻度数字右对齐 → 轴线）；**单横值轴、无左轴列的图表（蜂群图等）值轴自 x=0 铺满 0→1000——左缘即标题线，不留空轴边距，首尾刻度用 start/end 锚防溢出**；无轴图型（矩形树图等）格区直接铺满 0→1000。
- **高 = 内容底向上取整到 40 的倍数**，行 / 区块节奏 40。「内容底」含图例条自身（色块底）——带图例条的图，40 步进优先于「底部余量 ~10」（余量约 10 是无图例条的收底参考，两者冲突时步进让画布到 40 倍数，余量自然落在 ~4–36）。
- **宽高比是读数，不是门**：高度由上一条公式唯一决定，比值随内容多寡自然落位——同一类型不同规模会横跨区间（三阶段甘特 2.27、五阶段 1.47）。1.5–2.2 只是多数图的健康带，供 review 参考；出带不触发画布动作，也不为入带反调间距。要判断「该拆 / 该换类型」时查**内容结构**（密度、行数、任务数），按各 type 文档的复杂度预算执行。
- **内容多寡由间距消化**：加宽 / 收窄只动列距与 gap，节点尺寸与字号永不缩放；坐标一律 4 的倍数。
- **连线端点贴盒边**：line / path 的起终点落在目标盒（或容器、菱形顶点）的边线上，不埋进盒内、不悬在半空——箭头体（`refX=7`）留在盒外才可见（self_check 端点审计强制；hub-and-spoke 辐条压 hub 底下属设计豁免）。
- **内容不越出画布**：元素与连线端点的有效坐标（含 `<g transform>` 平移）保持在 viewBox 内——右 / 下溢出即红（self_check 越界闸）。
- **图例横线贯通 0→1000**：图例线 = 图块边界，绘图区右缘按图型家族惯例。架构 / 流程类（泳道列布局）内容右缘由列距与 gap 消化、不必顶到 1000，图例线仍贯通——底边满宽先于内容右缘。
...
**三档同构**：同一图的 标准 / dark / full 三档几何（viewBox、元素坐标、尺寸）逐元素一致，只有颜色按深色换档、文本各随其档——改任何一档的坐标必须同步其余两档。
...
系列多边形 / 面积的填充用 0.18 透明度（深色档 α 不动）；折线描边用全色。**不许回填到非图表类型**——架构图、泳道图等继续用 ink / muted 变体。**唯一例外：分类芯片编码**——data-flow / process 的载荷芯片（WB/DB/TB/FL/LS）与系列色同职能（图例为准的分类编码），映射见 [type-data-flow.md](type-data-flow.md) §8，两类型共用。系列色板是「重叠形状确实需要可区分颜色」时的 opt-in，不是到处加色的许可证。
...
| `terminal-ink` | `#f5f5f5` | 主文本、主描边（与浅色 ink 同一白） |
