...
页面 eyebrow / h1 的 `margin-left` = **图表内容最左标注元素的 x ÷ viewBox 宽**（百分比）。eyebrow、h1、竖排轴标题、图例横条区（图例标签 x）立在**同一条对齐线**上；图表内部（刻度 / 网格 / 节点几何）不动。

- 竖排文字基线补偿一个字高：`rotate(-90)` 后字形伸向基线左侧，基线 x = 对齐线 + 字号。
- **full 页面级例外**：eyebrow / h1 跟 `diagram-container` 框缘对齐，无 `margin-left`。
- 参照：bar 24/1000 = 2.4%、architecture 40/1000 = 4%、data-flow 24/872 ≈ 2.75%、dp-integration 40/1200 ≈ 3.33%。
