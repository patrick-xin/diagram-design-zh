...
1. 本文件总是载入。
2. **开工第一件事——样式门，先于一切类型工作**。禁令：**不许把默认皮肤的图悄悄交进一个有品牌的项目**。先查项目标记：`<项目根>/.diagram-design` 存在且合法时，按 [`references/profiles.md`](references/profiles.md)「每次生成前解析」直读品牌档案当生效指南（安装内工作副本一字不动），并跳过下面的首问门。否则首次制图前检查 `references/style-guide.md` 是否仍是出厂默认（`accent` 仍为出厂值）。若仍是默认、且用户语境是**真实实体的第一人称自述**（"我们公司 / 我们团队 / 我们的产品"——**不必报名字**，"我们是个两百人的制造企业"就算），**停下来先问，选完样式才开始画**：
...
字体已自托管（`fonts/` 内思源两族 GB2312 级 woff2 子集），**不外链 Google Fonts**——国内网络拉不到，外链还破坏单文件契约（self_check 拦截）。
